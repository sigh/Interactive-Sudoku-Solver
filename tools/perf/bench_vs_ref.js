...
//
// Usage:
//   node tools/perf/bench_vs_ref.js [--ref <git-ref>] [--require-identical] <benchmark_puzzles args...>
//
...
//   node tools/perf/bench_vs_ref.js --ref main --max-backtracks 200000 --solutions all \
//       --input "................................................................................." --repeat 5
...
// benchmark_puzzles.js moved tests/bench → tools/perf; resolve it per checkout
// so A/B against a pre-move baseline still runs that ref's own copy.
const SCRIPT_CANDIDATES = ['tools/perf/benchmark_puzzles.js', 'tests/bench/benchmark_puzzles.js'];
const scriptIn = (cwd) => {
  const found = SCRIPT_CANDIDATES.find((p) => existsSync(join(cwd, p)));
  if (!found) throw new Error(`no benchmark_puzzles.js under ${cwd}`);
  return found;
};
...
const usage = () => console.log(
  `Usage: node tools/perf/bench_vs_ref.js [--ref <git-ref>] [--require-identical] <benchmark_puzzles args...>\n\n` +
  ...
  `  <rest>               Forwarded to benchmark_puzzles.js (--max-backtracks is still required).\n\n` +
...
const main = () => {
  ...
  const refHasScript = SCRIPT_CANDIDATES.some((p) => {
    try {
      execFileSync('git', ['cat-file', '-e', `${args.ref}:${p}`], { cwd: root, stdio: 'ignore' });
      return true;
    } catch { return false; }
  });
  if (!refHasScript) {
    console.error(`error: ${args.ref} (${refSha}) does not contain benchmark_puzzles.js — ` +
      `bench_vs_ref needs the baseline revision to include the benchmark harness.`);
    process.exit(1);
  }
