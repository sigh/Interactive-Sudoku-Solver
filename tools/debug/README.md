...
| `node tools/debug/search_divergence.js` | Find the first differing selection, propagation outcome, conflict or solution between a baseline and a variant. |
...
### `search_divergence.js` — where two executions first differ

```sh
node tools/debug/search_divergence.js --puzzle 'Chaos Construction: 6x6' \
