...
await runTest('Indexing should fail init if control cell has no values within the line length', () => {
  ...
  const grid = context.grid;
  grid[0] = valueMask(9); // out of range for a 4-cell index list
  const result = context.initializeHandler(handler);
...
await runTest('Indexing init should work on non-standard grids (line length < numValues)', () => {
  ...
  assert.equal(result, true);
  assert.equal(grid[0], valueMask(1, 2, 3, 4, 5, 6), 'control should be limited to the line length');
});
...
await runTest('Indexing init should clamp a pre-restricted control cell', () => {
  ...
  const grid = context.grid;
  grid[0] = valueMask(2, 4, 9); // mix of in-range and out-of-range
  const result = context.initializeHandler(handler);

  assert.equal(result, true);
  assert.equal(grid[0], valueMask(2, 4), 'out-of-range candidates should be removed');
});
...
await runTest('Indexing init should be a no-op when line length equals numValues', () => {
  ...
  assert.equal(result, true);
  assert.equal(grid[0], valueMask(1, 2, 3, 4, 5));
});
...
await runTest('Indexing should prune control cell when indexed cell cannot have value', () => {
  ...
  grid[1] = valueMask(1, 2); // Indexed[0] - no 3
  grid[2] = valueMask(3, 4); // Indexed[1] - has 3
  grid[3] = valueMask(1, 2); // Indexed[2] - no 3
  grid[4] = valueMask(2, 3); // Indexed[3] - has 3
...
await runTest('Indexing should remove indexed value from cells that cannot be selected', () => {
  ...
  grid[1] = valueMask(1, 2, 3); // Indexed[0] - has 3, but control cannot be 1
  grid[2] = valueMask(3, 4); // Indexed[1] - selected (control=2)
  grid[3] = valueMask(1, 3); // Indexed[2] - has 3, but control cannot be 3
  grid[4] = valueMask(2, 3); // Indexed[3] - has 3, but control cannot be 4
  const acc = createAccumulator();
  ...
  assert.equal(grid[1], valueMask(1, 2), 'indexed[0] should not have 3');
  assert.equal(grid[2], valueMask(3, 4), 'indexed[1] keeps 3 (selected)');
  assert.equal(grid[3], valueMask(1), 'indexed[2] should not have 3');
  assert.equal(grid[4], valueMask(2), 'indexed[3] should not have 3');
});
...
await runTest('Indexing should fail when control has no valid options', () => {
  ...
  grid[1] = valueMask(1, 2); // Indexed[0] - no 3
  grid[2] = valueMask(1, 4); // Indexed[1] - no 3
  grid[3] = valueMask(1, 2); // Indexed[2] - no 3
  grid[4] = valueMask(2, 4); // Indexed[3] - no 3
  const acc = createAccumulator();
...
await runTest('Indexing should fail when removal empties an indexed cell', () => {
  ...
  grid[1] = valueMask(3); // Indexed[0] - only 3, but control ≠ 1
  grid[2] = valueMask(3); // Indexed[1] - only 3 (selected)
  grid[3] = valueMask(3); // Indexed[2] - only 3, but control ≠ 3
  grid[4] = valueMask(3); // Indexed[3] - only 3, but control ≠ 4
  const acc = createAccumulator();
...
await runTest('Indexing should work with fewer indexed cells than numValues', () => {
  ...
  const grid = context.grid;
  grid[0] = valueMask(1, 2, 3, 4, 5, 6, 7, 8); // Control - all values
  grid[1] = valueMask(1, 2, 3, 4, 5); // Indexed[0] - has 5
  grid[2] = valueMask(1, 2, 3, 4); // Indexed[1] - no 5
  grid[3] = valueMask(5, 6, 7, 8); // Indexed[2] - has 5
  grid[4] = valueMask(1, 2, 3); // Indexed[3] - no 5
  grid[5] = valueMask(4, 5, 6); // Indexed[4] - has 5
  grid[6] = valueMask(7, 8); // Indexed[5] - no 5
  ...
  assert.equal(grid[0], valueMask(1, 3, 5), 'control should allow 1, 3, 5');
});
...
await runTest('Indexing should work with more indexed cells than numValues', () => {
  ...
  grid[1] = valueMask(4); // Indexed[0] - has 4
  grid[2] = valueMask(1, 2, 3); // Indexed[1] - no 4
  grid[3] = valueMask(1, 2, 3); // Indexed[2] - no 4
  grid[4] = valueMask(1, 2, 3); // Indexed[3] - no 4
  grid[5] = valueMask(4, 5, 6); // Indexed[4] - has 4
  grid[6] = valueMask(1, 2, 3); // Indexed[5] - no 4
  grid[7] = valueMask(1, 2, 3); // Indexed[6] - no 4
  grid[8] = valueMask(1, 2, 3); // Indexed[7] - no 4
  const acc = createAccumulator();
  ...
  assert.equal(grid[0], valueMask(1, 5), 'control should only allow 1 and 5');
});
...
await runTest('Indexing should handle control restricted to valid indices', () => {
  ...
  grid[1] = valueMask(2, 3); // Indexed[0] - has 2
  grid[2] = valueMask(2, 3); // Indexed[1] - has 2
  grid[3] = valueMask(1, 3); // Indexed[2] - no 2
  grid[4] = valueMask(1, 3); // Indexed[3] - no 2
  const acc = createAccumulator();

  const result = handler.enforceConsistency(grid, acc);

  assert.equal(result, true);
  assert.equal(grid[0], valueMask(1, 2), 'control should allow 1 and 2');
});
...
await runTest('Indexing should handle single indexed cell', () => {
  ...
  grid[1] = valueMask(1, 2, 3); // Single indexed cell - has 3
  const acc = createAccumulator();
  ...
  assert.equal(grid[0], valueMask(1, 2, 3, 4), 'control can still be anything (only 1 indexes)');
...
await runTest('Indexing should handle all indexed cells having the value', () => {
  ...
  const grid = context.grid;
  grid[0] = valueMask(1, 2, 3, 4); // Control
  grid[1] = valueMask(1, 2, 3, 4); // All have 2
...
await runTest('Indexing should prune indexed cells based on control', () => {
  ...
  grid[1] = valueMask(1, 2, 3); // Indexed[0] - control=1 possible
  grid[2] = valueMask(1, 2, 3); // Indexed[1] - control=2 NOT possible
  grid[3] = valueMask(1, 2, 3); // Indexed[2] - control=3 possible
  grid[4] = valueMask(1, 2, 3); // Indexed[3] - control=4 NOT possible
  const acc = createAccumulator();
  ...
  assert.equal(grid[1], valueMask(1, 2, 3), 'indexed[0] keeps 3');
  assert.equal(grid[2], valueMask(1, 2), 'indexed[1] loses 3');
  assert.equal(grid[3], valueMask(1, 2, 3), 'indexed[2] keeps 3');
