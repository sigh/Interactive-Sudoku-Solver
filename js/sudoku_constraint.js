...
export class StateCellRegistry {
  ...
  addGroup(prefix, label, cells, { hidden = false } = {}) {
    this._groups.push({ prefix, label, cells: [...cells], hidden });
    for (let i = 0; i < cells.length; i++) {
      const id = cells.length === 1 ? prefix : `${prefix}${i}`;
      this._cellToId.set(cells[i], id);
      this._idToCell.set(id, cells[i]);
    }
  }
  ...
  getCellsForGroup(prefix) {
    for (const group of this._groups) {
      if (group.prefix === prefix) return group.cells;
    }
    return null;
  }
  ...
  getCellIndex(cellId) {
    return this._idToCell.has(cellId) ? this._idToCell.get(cellId) : null;
  }
  ...
  static fromJSON(json) {
    const registry = new StateCellRegistry();
    for (const group of json) {
      registry.addGroup(group.prefix, group.label, group.cells,
        { hidden: group.hidden });
    }
    return registry;
  }
  ...
  // Constraints with a static getStateCellGroups method declare their groups.
  // Groups are sorted alphabetically by prefix to ensure deterministic
  // cell index allocation.
  ...
  // Build a registry from group specs, allocating cell indices sequentially
  // starting from numCells. Specs are sorted by prefix for determinism.
  static fromGroupSpecs(specs, numCells) {
    specs.sort((a, b) => a.prefix < b.prefix ? -1 : a.prefix > b.prefix ? 1 : 0);

    const registry = new StateCellRegistry();
    let next = numCells;
    for (const spec of specs) {
      const cells = Array.from({ length: spec.count }, (_, i) => next + i);
      next += spec.count;
      registry.addGroup(spec.prefix, spec.label, cells,
