...
- **UI** ([constraint_input.js](ui/constraint_input.js), [constraint_display.js](ui/constraint_display.js), [display.js](ui/display.js), [constraint_collection.js](ui/constraint_collection.js), [grid_input.js](ui/grid_input.js)) — SVG grid rendering, constraint visualization, input controls, constraint chips, and grid selection.
- **Solver** ([solver/](solver/)) — A constraint-satisfaction engine that runs in a Web Worker. See [solver/README.md](solver/README.md).
- **Orchestration** ([render_page.js](ui/render_page.js), [solution_controller.js](ui/solution_controller.js), [solver_runner.js](ui/solver_runner.js)) — Wires everything together: manages state, coordinates solving, and handles URL-based history.

### Entry Point

[index.html](../index.html) loads scripts as ES modules and calls `initPage()` from [render_page.js](ui/render_page.js) on load. Scripts are preloaded asynchronously (except on Safari, where they load synchronously).
...
The `ConstraintManager` (defined in [render_page.js](ui/render_page.js)) is the central hub. It holds the active constraint tree and notifies listeners when it changes.

1. **Input**: The user interacts with [constraint_input.js](ui/constraint_input.js) controls, which create constraint objects (from [sudoku_constraint.js](sudoku_constraint.js)) and add them to the `ConstraintManager`.
2. **Display**: On each update, [constraint_display.js](ui/constraint_display.js) re-renders constraints as SVG overlays on the grid managed by [display.js](ui/display.js).
3. **Solving**: [solution_controller.js](ui/solution_controller.js) receives the constraint tree from the manager and passes it to [solver_runner.js](ui/solver_runner.js), which sends it to [solver_worker.js](solver_worker.js) via `postMessage`. The worker builds and runs the solver engine ([solver/](solver/)).
4. **Results**: Solutions flow back through `postMessage` and are rendered via [display.js](ui/display.js).
...
The top level holds the constraint model and the worker entry points, which
the workers, sandbox and tools share. [ui/](ui/) holds the browser page's
main-thread code: only index.html, other ui/ files and [debug/](debug/)
import from it.

| File | Purpose |
|------|---------|
| [ui/render_page.js](ui/render_page.js) | **Entry point.** Initializes all components, wires up event listeners, configures the bottom drawer. Also defines `ConstraintManager`, which holds the active constraint tree and notifies listeners on changes. |
| [ui/constraint_collection.js](ui/constraint_collection.js) | The constraint collections `ConstraintManager` adds to: the root, composites (Or/And/Replicate), and the redirect to a selected composite. Also the chips (`ConstraintChipView`) and constraint selection (`ConstraintSelector`). |
| [ui/grid_input.js](ui/grid_input.js) | `GridInputManager`: cell selection on the grid (drag, shift, rectangle, segments) and typing into cells. |
| [ui/display.js](ui/display.js) | SVG rendering system. `DisplayContainer` manages layered SVG groups. Key classes: `CellValueDisplay`, `SolutionDisplay`, `HighlightDisplay`, `GridDisplay`, `BorderDisplay`, `ColorPicker`. |
| [ui/constraint_display.js](ui/constraint_display.js) | Renders constraints as SVG overlays (lines, regions, shading, arrows, dots, etc.). Display item subclasses inherit from `BaseConstraintDisplayItem`. |
| [ui/constraint_input.js](ui/constraint_input.js) | UI controls for adding/configuring constraints. `CollapsibleContainer` for grouped inputs. Auto-saves configuration state. |
...
| [ui/solution_controller.js](ui/solution_controller.js) | Bridges the constraint manager and solver. Manages solve modes, URL-based history (undo/redo), debug/flame-graph integration, and keyboard shortcuts. |
| [ui/solver_runner.js](ui/solver_runner.js) | Solve mode strategies: `AllPossibilitiesModeHandler`, `AllSolutionsModeHandler`, `StepByStepModeHandler`, `CountModeHandler`, `EstimatedCountModeHandler`. Each mode controls how the worker is invoked and results are processed. |
| [ui/solver_state_display.js](ui/solver_state_display.js) | The solver state panel: solve status, counters, progress bar, and the stats charts (loaded on demand). |
...
| [cell_geometry.js](cell_geometry.js) | Grid coordinate system. `CellGeometry` handles cell indexing (R1C1 format), value ranges, and grid dimensions (1×1 to 16×16). `CellGraph` provides adjacency queries. `VarCellRegistry` manages named variable cells outside the grid. |
...
| [ui/bottom_drawer.js](ui/bottom_drawer.js) | Tabbed panel UI below the grid. `BottomDrawer` manages tabs (debug, flame graph). `LazyDrawerManager` loads tab modules on demand. |
...
|-----------|---------|--------|
| [ui/](ui/) | The page: rendering, constraint input, grid selection, and running the solver from the main thread | This file |
...
- **SVG rendering**: All grid visuals use SVG elements managed by [display.js](ui/display.js). Display items are layered into named `<g>` groups. Layout updates are batched via `deferUntilAnimationFrame`.
