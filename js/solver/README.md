...
| [chaos_handler.js](chaos_handler.js) | **Chaos construction.** Discovers an unknown region (box) layout: partitions the grid into connected, distinct-value regions. See [chaos_construction.md](handler_docs/chaos_construction.md). |
| [connected_handler.js](connected_handler.js) | **Value connectivity.** `ConnectedValues` enforces that the cells holding one of a set of values form a single orthogonally-connected region. Single-BFS check plus one-door forcing. See [connected_values.md](handler_docs/connected_values.md). |
