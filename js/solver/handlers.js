...
class SudokuConstraintOptimizer {
  static _HOUSE_SUM = GRID_SIZE*(GRID_SIZE+1)/2;
  ...
  static _fillInSumGap(sumHandlers, sumCells) {
    ...
    const remainingSum = GRID_SIZE*this._HOUSE_SUM - sumHandlersSum;
  ...
  static _addSumIntersectionHandler(
      handlerSet, houseHandler, overlappingHandlerIndexes) {
    ...
    const outsideSum = totalSum - this._HOUSE_SUM;
  ...
  static _makeHiddenCageHandlers(handlerSet, sumHandlers) {
    ...
    for (const h of houseHandlers) {
      ...
      const complementSum = this._HOUSE_SUM - constrainedSum;
  ...
  static _makeInnieOutieSumHandlers(sumHandlers, hasBoxes) {
    ...
    const handleOverlap = (superRegion, piecesRegion, usedPieces) => {
      ...
      let sumDelta = -superRegion.size*this._HOUSE_SUM/GRID_SIZE;
