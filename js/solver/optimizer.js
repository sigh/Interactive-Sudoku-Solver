...
const { memoize, arrayIntersectSize, arrayDifference, setIntersectSize, arrayIntersect, arrayRemoveValue, setIntersectionToArray, setDifference } = await import('../util.js' + versionParam);
const { binaryFnToKey, LookupTables } = await import('./lookup_tables.js' + versionParam);
const { SudokuConstraintBase } = await import('../sudoku_constraint.js' + versionParam);
...
export class SudokuConstraintOptimizer {
  ...
  static _equalsKey = memoize((numValues) => binaryFnToKey(
    (a, b) => a == b, numValues));
